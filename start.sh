...
record_start() {
	...
	ansible-playbook -i $INVENTORY ${ROOT}/ansible/playbook.yml
}

record_stop() {
	counter=$1
	ssh -i $PRIV_KEY root@`retrueve_ip` 'killall -INT ffmpeg'
	scp -i $PRIV_KEY root@`retrieve_ip`:/root/reg.mp4 $ROOT/regs/${NOME_CORSO}-${ANNO}-${ID}_${counter}.mp4
	cd terraform
	terraform destroy -var="anno=$ANNO" -var="corso=$NOME_CORSO" -state $TFSTATE
	cd $ROOT
}
